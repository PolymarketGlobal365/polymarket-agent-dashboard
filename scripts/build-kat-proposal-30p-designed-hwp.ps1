...
$rendererPath = Join-Path $PSScriptRoot 'render-kat-proposal-30p-pages.mjs'
$rendererPath = [System.IO.Path]::GetFullPath($rendererPath)
$inputDir = Join-Path $PSScriptRoot '..\output\kat_30p_pages'
$inputDir = [System.IO.Path]::GetFullPath($inputDir)
$outputPath = Join-Path $PSScriptRoot '..\output\proposal_2026_korean_artist_today_30p_designed.hwp'
